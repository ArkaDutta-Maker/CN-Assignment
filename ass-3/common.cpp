...
// Example 6-byte addresses
const uint8_t SENDER_ADDR[6] = {0xAA, 0xBB, 0xCC, 0x11, 0x22, 0x33};
const uint8_t RECEIVER_ADDR[6] = {0xDE, 0xAD, 0xBE, 0xEF, 0x44, 0x55};

// Standard-ish generator polys for demonstration
// (non-reflected, initial=0, no final XOR to keep both sides simple and consistent)
...
uint32_t compute_crc(const std::vector<uint8_t> &bytes, int widthBits)
{
    ...
    // Pack into 32 bits LSBits; we’ll send as 4 bytes big-endian
